...
contract ZapApyV2Pro is Ownable, ReentrancyGuard {
    ...
    // --- PROFIT & PENALTY RULES ---
    uint256 public constant REFERRAL_SHARE = 10; // 10% of PROFIT
    ...
    struct UserInfo {
        ...
        uint256 lastDepositTime; // Tracks the 76-hour lock
    }
    ...
    function zapIn(uint256 _amount, address _referrer) external nonReentrant {
        ...
        user.lastDepositTime = block.timestamp; // Start Lock Timer
    }
    ...
    function zapOut() external nonReentrant {
        ...
        // --- PENALTY CHECK (Early Exit) ---
        if (block.timestamp < depositTime + LOCK_TIME) {
            // 1% Penalty applied to total value. No profit calculation.
        ...
        // --- STANDARD PROFIT CHECK (Post-76 Hours) ---
        uint256 profit = 0;
        ...
        if (profit > 0) {
            // Referrer gets 10% of PROFIT
            uint256 refPayout = (profit * REFERRAL_SHARE) / 100;
            // Protocol gets 5% of PROFIT
            uint256 ownerFee = (profit * PROTOCOL_FEE) / 100;
            // User gets 85% of PROFIT + their original Principal
